...
Shader::Shader(const std::string& vertexShaderPath, const std::string& fragmentShaderPath): programID(0) {
    ...
    try {
        ...
        std::runtime_error("Failed to load the shader\n");
    }
...
void Shader::checkShader(const uint32_t shader) const {
    // check compilation
    int success;
    char infoLog[512];
    ...
    if (!success) {
        ...
        std::cerr << "Couldn't check shader compilation: " << infoLog << std::endl;
        EXIT_FAILURE;
    }
}

void Shader::checkProgram(const uint32_t programID) const {
    // check linking
    int success;
    char infoLog[512];
    ...
    if (!success) {
        ...
        std::cerr << "Couldn't check program linking: " << infoLog << std::endl;
        EXIT_FAILURE;
    }
}